...
db.read().then(async (data: any) => {
  ...
  try {
    ...
    list = res.edges.map((item: {
      node:
        & Record<"title" | "uuid" | "summary" | "createdAt" | "slug", string>
        & {
      ...
      return {
        ...
        link:
          `https://leetcode-cn.com/problems/${item.node.question.questionTitleSlug}/solution/${item.node.slug}/`,
      };
    });
  } catch (e) {
    throw Error(`
